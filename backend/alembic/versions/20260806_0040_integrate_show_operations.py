...
def upgrade() -> None:
    ...
    op.create_index("idx_tasks_source_incident_id", "tasks", ["source_incident_id"])
    ...
    op.execute("create policy evidences_update on evidences for update using (app_is_admin() or (app_current_role()='SUPERVISOR' and app_can_view_event(event_id)) or uploaded_by=app_current_user_id()) with check (app_is_admin() or (app_current_role()='SUPERVISOR' and app_can_view_event(event_id)) or uploaded_by=app_current_user_id())")
    op.execute("create policy evidences_delete on evidences for delete using (app_is_admin() or (app_current_role()='SUPERVISOR' and app_can_view_event(event_id)) or uploaded_by=app_current_user_id())")
...
def downgrade() -> None:
    ...
    op.drop_index("idx_tasks_source_incident_id", table_name="tasks")
