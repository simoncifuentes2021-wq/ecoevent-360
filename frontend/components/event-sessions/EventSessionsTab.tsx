...
export function EventSessionsTab({ eventId, role }: { eventId: string; role?: UserRole | null }) {
  ...
  return <div className="space-y-4">
    ...
    {!loading && !error ? <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{items.map((item, index) => <article className={`rounded-lg border bg-white p-4 shadow-sm ${item.archived_at ? "opacity-60" : ""}`} key={item.id}>
    ...
    {detail ? <SessionDetail item={detail} responsible={detail.responsible_id ? staffNames.get(detail.responsible_id) : undefined} canManage={canManage && !detail.archived_at} onClose={() => setDetail(null)} onTransition={(status) => { void changeStatus(detail, status); setDetail(null); }} /> : null}
