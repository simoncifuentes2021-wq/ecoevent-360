  ...
  return <ModalShell title={action.name} description={`${showName || "Evento completo"} · Detalle trazable del cálculo`} onClose={onClose} size="lg">
    <div className="space-y-6">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Info icon={Zap} label="Energía utilizada" value={`${number(energy?.value ?? action.energy_kwh ?? null)} kWh`} />
        <Info icon={Fuel} label="Combustible evitado" value={fuel?.value == null ? "No calculado" : `${number(fuel.value)} L`} />
