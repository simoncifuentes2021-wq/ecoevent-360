    ...
    <ModalShell title={survey.title} description="Configuracion de encuesta externa y enlaces asociados." onClose={onClose}>
      <div className="space-y-4 text-sm">
        ...
        <div className="grid gap-2 rounded-xl bg-slate-50 p-3">
          <span>Apertura: {survey.opens_at ? new Date(survey.opens_at).toLocaleString("es-CL", { timeZone: "America/Santiago" }) : "Sin fecha"}</span>
          <span>Cierre: {survey.closes_at ? new Date(survey.closes_at).toLocaleString("es-CL", { timeZone: "America/Santiago" }) : "Sin fecha"}</span>
