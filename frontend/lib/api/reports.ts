...
export function applyReportAssistantProposal(reportId: string, generationId: string, editVersion: number, acceptedSectionKeys?: string[], applyPreset = true) { return api.post<{ revision_id: string; report: ReportEditor; visual_audit?: { initial: { passed: boolean; browser_checked: boolean; findings: Array<{ code: string; severity: string; message: string; page?: number | null; section_keys: string[] }> }; corrections_applied: string[]; final: { passed: boolean; browser_checked: boolean; findings: Array<{ code: string; severity: string; message: string; page?: number | null; section_keys: string[] }> } } }>(`/reports/${reportId}/ai-assistant/proposals/apply`, { generation_id: generationId, edit_version: editVersion, accepted_section_keys: acceptedSectionKeys ?? null, apply_preset: applyPreset }); }
...
export function getReportAIMonthlyUsage() { return api.get<import("@/types/report").ReportAIUsageSummary>("/reports/ai-usage/monthly"); }
