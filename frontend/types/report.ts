...
export type ReportVisualPreset = "AUTO" | "ECOEVENT_EDITORIAL" | "EXECUTIVE" | "ENVIRONMENTAL" | "BIKE_ZONE" | "IMPACT";
export type ReportVisualVariant = "AUTO" | "IMPACT_STORY" | "SHOW_COMPARISON" | "CARD_GRID" | "SECTION_HERO" | "PHOTO_STORY";
export type ReportVisualConfig = { preset: ReportVisualPreset; icon_density: "LOW" | "MEDIUM" | "HIGH"; visual_density: "AIRY" | "BALANCED" | "COMPACT"; show_icons: boolean; show_trends: boolean; show_equivalences: boolean };
export type ReportSectionVisualConfig = { variant: ReportVisualVariant; icon_key?: "LEAF" | "RECYCLE" | "BICYCLE" | "CARBON" | "ENERGY" | "WATER" | "PEOPLE" | "LOCATION" | "CALENDAR" | "CHART" | "CHECK" | "ALERT" | "LIGHTBULB" | "CAMERA" | "TARGET" | null; show_icon: boolean; show_trend: boolean; premium_variant?: string; emphasis?: "LOW" | "NORMAL" | "HIGH"; selected_metric_keys?: string[] };
export type ReportEditorialConfig = { mode: "AUTO" | "CUSTOM"; cover_style: "FULL_PHOTO" | "SIDE_PHOTO" | "EDITORIAL" | "MINIMAL_PREMIUM"; cover_evidence_id?: string | null; cover_show_photo?: boolean; target_page_count?: number | null; featured_kpi_ids: string[]; page_overrides: Record<string, { mode: ReportPageOverrideMode; group_with?: string | null }>; chart_types: Record<string, "BAR" | "DONUT" | "COMPARISON" | "DISTRIBUTION">; visual_config: ReportVisualConfig; section_visuals: Record<string, ReportSectionVisualConfig> };
...
export type ReportAIClaim = { text: string; source_keys: string[] };
export type ReportAIAssistantProposal = { version: "1"; report_id: string; recommended_preset: ReportVisualPreset; rationale: string; estimated_pages: number; sections: Array<{ section_key: string; visible: boolean; order: number; premium_variant: string; emphasis: "LOW" | "NORMAL" | "HIGH"; narrative?: string | null; findings: ReportAIClaim[]; selected_metric_keys: string[]; selected_evidence_ids: string[]; page_mode: ReportPageOverrideMode; group_with?: string | null }>; executive_summary: ReportAIClaim[]; key_findings: ReportAIClaim[]; recommendations: Array<{ title: string; description: string; priority: "LOW" | "MEDIUM" | "HIGH"; source_keys: string[]; origin: "AI_GENERATED_RECOMMENDATION" }>; conclusion: ReportAIClaim[]; evidence_suggestions: Array<{ evidence_id: string; section_key: string; rationale: string }>; warnings: string[]; source_keys: string[]; generation_id: string; provider: string; model: string; effective_model?: string | null; prompt_version: string; cached: boolean; generated_at: string; input_tokens: number; output_tokens: number; cached_input_tokens: number; actual_cost_usd?: number | null; diff: Array<{ type: string; key: string; current: unknown; proposed: unknown }> };
export type ReportAIGenerationHistoryItem = { id: string; created_at: string; capability: string; provider: string; model: string; effective_model?: string | null; status: string; cached: boolean; applied: boolean; input_tokens: number; output_tokens: number; cached_input_tokens: number; actual_cost_usd?: number | null };
export type ReportAIUsageSummary = { month: string; spent_usd: number; budget_usd: number; remaining_usd: number; generation_count: number };
